...
OpLogBuilder是一个操作日志生成工具，旨在为Java生成操作日志提供一个简化的生成方案。

假设某个有网站存放了一些用户的个人信息，每个用户的个人信息存放在下面的实体中：
...
某一天，这个网站需要记录用户每次更新的操作日志，那么则需要这么写代码：
...
public class OpLogConfig {
    ...
    @Bean
    public OpLogBuilder opLogBuilder() {
        // 定义builder
        OpLogBuilder builder = new OpLogBuilderImpl();
...
### `OpLogConfig`详解（含`@OpLogBean`, `@OpLogField`, `@OpLogBeanInside`）

#### 1. `OpLogConfig`各个字段的含义

`OpLogConfig`顾名思义是日志生成时的配置信息类。整个日志生成过程中，几乎所有的配置信息都存放在`OpLogConfig`中。

`OpLogConfig`每个字段的含义如下表。

|字段名|字段说明|全局默认|设置类型|
|:---:|:---------|:---|:---:|
|name|记录到操作日志中的属性名称<br>若为null则使用变量的field名称<br>|`null`|`String`|
|logExp|单个field生成的提示文本<br>使用SpEl表达式填充<br>|`{#name}: #{#before}->#{#after}`|`String`|
|effectiveType|日志生效的变更类型<br>在这个集合中未包含的日志不会记录|`{OpLogChangeType.UPDATE, OpLogChangeType.INSERT, OpLogChangeType.DELETE}`|`OpLogChangeType[]`|
...
#### 2. `@OpLogBean`, `@OpLogField`, `@OpLogBeanInside`与`OpLogConfig`的对应关系

`@OpLogBean`,`@OpLogField`,`@OpLogBeanInside`会在生成日志的时候转换为对应的`OpLogConfig`类对象，

|`OpLogConfig`<br>包含属性|`@OpLogBean`<br>是否支持|`@OpLogField`<br>是否支持|`@OpLogBeanInside`<br>是否支持|
|:---:|:---:|:---:|:---:|
